...
print(f"In ciocc.py, logfile is {logfile.name}", file=sys.stderr)
logging.basicConfig(filename=logfile.name, filemode="w", level=logging.DEBUG)
l = logging
...
checker_plugin_path = Path("~/projects/checker/bap/interval/").expanduser()
...
class Command():
    ...
    def __run_cmd(self, cmd: List[str]):
        try:
            subprocess.run(cmd, timeout=60, env=os.environ)
